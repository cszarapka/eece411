...
//DELETE NODES.TXT
//CREATE RESULTS.TXT
$resultFile = file_put_contents('results'.$hostname.'.txt', $hostname."|2|");
...
file_put_contents('results'.$hostname.'.txt', $diskUsed.'|'.$diskAvail.'|'.$upTime."\n" , FILE_APPEND);
...
foreach( $nodesArray as $key => $slice )
{
	...
	{
		echo 'Yes: '.$slice."\n\n";
		$command = 'ssh -o StrictHostKeyChecking=no -l ubc_eece411_5 -i id_rsa '.$slice.' "sudo yum install php -y"';
		...
		if( $ping1 == '' and stristr($result, 'Setting up') != false)
		{
			echo 'Found Server';
			$ping1 = $slice;
			file_put_contents("sendingTo.txt",$ping1);
			//WRITE $SLICE TO RESULTS.TXT + ONLINE
		} 
		else 
		{
			file_put_contents('results'.$hostname.'.txt', $slice."|1\n" , FILE_APPEND);
		}

		
		if( $ping1 != '' && $ping1 != $slice && $ping2 == ''  and stristr($result, 'Setting up') != false)
		{
			echo 'Found Server';
			$ping2 = $slice;
			file_put_contents("sendingTo.txt",$ping2, FILE_APPEND);
			//WRITE $SLICE TO RESULTS.TXT + ONLINE
			break;
		}
		elseif($ping1 != '') 
	...
	{
		echo 'No: '.$slice."\n\n";
		file_put_contents( 'results'.$hostname.'.txt', $slice."|0\n", FILE_APPEND );
...
//for( $i = )
if($ping1 != '') {
	file_put_contents("sendingTo.txt",$ping1, FILE_APPEND);
	$array1String = trim( implode("\n", $array1) );
	echo $array1String;
	file_put_contents( 'nodes.txt', $array1String );
	//upload ssh shit
	echo "\n";
	$command = 'scp -i id_rsa \'nodes.txt\' id_rsa id_rsa.pub monitorService.php ubc_eece411_5@'.$ping1.':';
	echo $command."\n";
	system($command);
	$command = 'ssh -o StrictHostKeyChecking=no -l ubc_eece411_5 -i ~/.ssh/id_rsa '.$ping1.' "sudo php monitorService.php > /dev/null 2>&1 &"';
	system($command);
}
echo "\n\n\n";
...
if($ping2 != '') {
	file_put_contents("sendingTo.txt", "\n".$ping2, FILE_APPEND);
	$array2String = trim( implode("\n", $array2) );
	file_put_contents( 
		'nodes.txt', $array2String );
	echo $array2String;
	$command = 'scp -i id_rsa \'nodes.txt\' id_rsa id_rsa.pub monitorService.php ubc_eece411_5@'.$ping2.':';
	echo $command."\n";
	system($command);
	$command = 'ssh -o StrictHostKeyChecking=no -l ubc_eece411_5 -i id_rsa '.$ping2.' "sudo php monitorService.php > /dev/null 2>&1 &"';
	system($command);
	//file_put_contents( 'nodes.txt', $array2 );
	//upload ssh shit
}

$command = 'scp -i id_rsa results'.$hostname.'.txt ubc_eece411_5@planetlab2.cs.ubc.ca:';
system($command);
